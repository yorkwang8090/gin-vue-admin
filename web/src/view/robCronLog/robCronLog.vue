<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="交易所" prop="exid" width="120" />
        <el-table-column align="left" label="交易对" prop="pid" width="120" />
        <el-table-column align="left" label="交易对" prop="symbol" width="120" />
        <el-table-column align="left" label="类型 对敲  趋势  跟随" prop="type" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateRobCronLogFunc(scope.row)">变更</el-button>
       ...
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="交易所:"  prop="exid" >
              <el-input v-model.number="formData.exid" :clearable="true" placeholder="请输入交易所" />
            </el-form-item>
            <el-form-item label="交易对:"  prop="pid" >
              <el-input v-model.number="formData.pid" :clearable="true" placeholder="请输入交易对" />
            </el-form-item>
            ...
            <el-form-item label="addtime字段:"  prop="addtime" >
              <el-input v-model.number="formData.addtime" :clearable="true" placeholder="请输入addtime字段" />
            </el-form-item>
          </el-form>
    </el-drawer>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="类型 对敲  趋势  跟随">
                        {{ formData.type }}
                </el-descriptions-item>
                <el-descriptions-item label="内容">
                        {{ formData.content }}
                </el-descriptions-item>
                <el-descriptions-item label="addtime字段">
                        {{ formData.addtime }}
                </el-descriptions-item>
        </el-descriptions>
    </el-drawer>
...
const getTableData = async() => {
  const table = await getRobCronLogList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteRobCronLogByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRobCronLogFunc = async(row) => {
    const res = await findRobCronLog({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRobCronLogFunc = async (row) => {
    const res = await deleteRobCronLog({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
