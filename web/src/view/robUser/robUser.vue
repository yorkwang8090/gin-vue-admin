<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="登陆账号" prop="userName" width="120" />
        <el-table-column align="left" label="英文昵称" prop="userNickName" width="120" />
        <el-table-column align="left" label="登陆密码" prop="userPwd" width="120" />
        <el-table-column align="left" label="通知邮件" prop="mobile" width="120" />
        <el-table-column align="left" label="添加时间" prop="addtime" width="120" />
        <el-table-column align="left" label="exptime字段" prop="exptime" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateRobUserFunc(scope.row)">变更</el-button>
       ...
       <template #title>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="主键:"  prop="id" >
              <el-select v-model="formData.id" placeholder="请选择主键" style="width:100%" :clearable="true" >
            ...
            <el-form-item label="exptime字段:"  prop="exptime" >
              <el-input v-model.number="formData.exptime" :clearable="true" placeholder="请输入exptime字段" />
            </el-form-item>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                <el-descriptions-item label="主键">
                        {{ filterDict(formData.id,intOptions) }}
                </el-descriptions-item>
                <el-descriptions-item label="登陆账号">
                        {{ formData.userName }}
                </el-descriptions-item>
                <el-descriptions-item label="英文昵称">
                        {{ formData.userNickName }}
                </el-descriptions-item>
                <el-descriptions-item label="登陆密码">
                        {{ formData.userPwd }}
                </el-descriptions-item>
                <el-descriptions-item label="通知邮件">
                        {{ formData.mobile }}
                </el-descriptions-item>
                <el-descriptions-item label="添加时间">
                        {{ formData.addtime }}
                </el-descriptions-item>
                <el-descriptions-item label="exptime字段">
                        {{ formData.exptime }}
                </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getRobUserList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteRobUserByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRobUserFunc = async(row) => {
    const res = await findRobUser({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRobUserFunc = async (row) => {
    const res = await deleteRobUser({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
