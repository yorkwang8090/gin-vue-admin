<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="交易所名称" prop="title" width="120" />
        <el-table-column align="left" label="api接口地址" prop="apiUrl" width="120" />
        <el-table-column align="left" label="扩展参数，如comexvip的uid：156" prop="extend" width="120" />
        <el-table-column align="left" label="是否为跟随交易所" prop="isfollow" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.isfollow) }}</template>
        </el-table-column>
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateRobExchangeFunc(scope.row)">变更</el-button>
    ...
    <el-drawer size="800" v-model="detailShow" :before-close="closeDetailShow" title="查看详情" destroy-on-close>
        ...
        <el-descriptions :column="1" border>
                ...
                <el-descriptions-item label="api接口地址">
                        {{ formData.apiUrl }}
                </el-descriptions-item>
                <el-descriptions-item label="扩展参数，如comexvip的uid：156">
                        {{ formData.extend }}
                </el-descriptions-item>
                <el-descriptions-item label="是否为跟随交易所">
                    {{ formatBoolean(formData.isfollow) }}
                </el-descriptions-item>
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    ...
    page.value = 1
    pageSize.value = 10
    if (searchInfo.value.isfollow === ""){
        searchInfo.value.isfollow=null
    }
...
const getTableData = async() => {
  const table = await getRobExchangeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteRobExchangeByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateRobExchangeFunc = async(row) => {
    const res = await findRobExchange({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteRobExchangeFunc = async (row) => {
    const res = await deleteRobExchange({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
